# Preprocessing script from the CovidTwitterBertv2 repo
# see: https://github.com/digitalepidemiologylab/covid-twitter-bert/blob/master/utils/preprocess.py
...
# Fillers from CovidBert Config
# see: https://github.com/digitalepidemiologylab/covid-twitter-bert/blob/86909698f104525b13d8ab0e22ae666e264ac5c6/preprocess/create_finetune_data.py#L168
...
def segment_sentences(text, args):
    ...
    doc = nlp(text)
    regex_fillers = r"(^\d {username}$)|^{username}$|(^\d {url}$)|^{url}$".format(
        username=args.username_filler, url=args.url_filler
    )
    num_tokens = len(doc)
...
# For easily passing arguments to the preprocessing function
class Args:
    def __init__(self, **kwargs):
        self.__dict__.update(kwargs)


# Arguments to pass to the preprocessing function
args = Args(
    replace_usernames=True,
    username_filler=USERNAME_FILLER,
    replace_urls=True,
    url_filler=URL_FILLER,
    asciify_emojis=True,
...
preprocess = partial(preprocess_bert, args=args)
